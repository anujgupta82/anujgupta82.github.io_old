...
$$
\begin{align}
\frac{\partial L}{\partial w_1} &= \sum\limits_{i=1}^{4} (y_i - \hat{y}_i) *  [\sigma( X_i . [W])(1 - \sigma( X_i . [W])) \times x_{i}^{1}]
\end{align}
$$

Likewise, we can compute \\( \frac{\partial L}{\partial w_{2}} \\) and \\( \frac{\partial L}{\partial w_{3}} \\)
