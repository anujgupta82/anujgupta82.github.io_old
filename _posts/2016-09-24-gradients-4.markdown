...
\frac{\partial l_1}{\partial w_1^1} &= \big{(} \frac{1}{1 + e^{-[X] . [W_1]}} \big{)} \times \big{(}1- \frac{1}{1 + e^{-[X] . [W_1]}} \big{)} * -x_1 \label{ref25} \tag{15}\\
& = l_1 \times (1- l_1) * -x_1 \dots && \text{using \eqref{ref11}} \label{ref26} \tag{16}\\
...
$$
\begin{align}
\frac{\partial L}{\partial w_{1}^{1}} & = \big(-(y - \hat{y}) \big) \times \big( \hat{y} \times (1- \hat{y}) * -w_2 \big) \times  \big( l_1 \times (1- l_1) * -x_1 \big) \\
& = \big((y - \hat{y})\hat{y}(1- \hat{y}) * w_2 \big) \times  \big( l_1 \times (1- l_1) * -x_1 \big) \\

& = \big( \Delta l_{2} \times  w_2 \times l_1 \times (1- l_1) \big) \times -x_1  \dots && \text{using \eqref{ref17}} \label{ref27} \tag{17}\\
